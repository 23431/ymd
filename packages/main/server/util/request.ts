...
const createRequest = (method: any, url: any, data: any, options: any) => {
    return new Promise((resolve, reject) => {
        let headers = {'User-Agent': chooseUserAgent(options.ua)}
        if (method.toUpperCase() === 'POST') { // @ts-ignore
            headers['Content-Type'] = 'application/x-www-form-urlencoded'
        }
        if (url.includes('music.163.com')) { // @ts-ignore
            headers['Referer'] = 'https://music.163.com'
        }
        if (options.realIP) { // @ts-ignore
            headers['X-Real-IP'] = options.realIP
        }
        // headers['X-Real-IP'] = '118.88.88.88'
        if (typeof options.cookie === 'object') { // @ts-ignore
            headers['Cookie'] = Object.keys(options.cookie)
                ...
                .join('; ')
        } else if (options.cookie) { // @ts-ignore
        ...
        if (options.crypto === 'weapi') {
            ...
            if (cookie.MUSIC_U) { // @ts-ignore
                header['MUSIC_U'] = cookie.MUSIC_U
            }
            if (cookie.MUSIC_A) { // @ts-ignore
        ...
        let settings = {
            ...
            httpsAgent: new https.Agent({keepAlive: true}), proxy: undefined

        }

        if (options.crypto === 'eapi') { // @ts-ignore
        ...
        axios(settings)
            .then((res: { data: any; headers: { [x: string]: any }; status: number }) => {
                const body = res.data
                // @ts-ignore
